...
Бот отправляет **оффлайн-конверсии** в Метрику на каждом ключевом шаге воронки.
Все цели задаются в `.env` (переменные `GOAL_*`) и отправляются при соответствующих действиях.
...
Где `1234567890` — значение cookie `_ym_uid` (ClientId) пользователя.

Важно: если ClientId не передан, бот **не отправляет** оффлайн-конверсии,
потому что без ClientId Метрика не сможет связать событие с визитом.
...
| `ADMIN_USER_ID` | Ваш Telegram user id (личка) для debug-ответов с `file_id` |
| `METRIKA_TOKEN` | OAuth-токен Яндекс.Метрики |
| `METRIKA_COUNTER_ID` | ID счётчика Метрики |
| `GOAL_BOT_STARTED` | ID цели «Бот запущен» |
| `GOAL_*` | ID целей воронки (например `GOAL_STEP1`, `GOAL_PRICE`, `GOAL_ORDER_WITH_PHONE` и т.д.) |
...
Проще всего — отправить боту видео/кружок в личку от пользователя,
чей `user id` прописан в `ADMIN_USER_ID`: бот ответит `file_id`.

Примечание: если тестируете в группе и у бота включён privacy mode,
он может не получать обычные сообщения/медиа из группы.
